...
import { Award, CheckCircle2, HelpCircle, Send, Sparkles, AlertCircle, RefreshCw } from "lucide-react";
...
export default function MandatoryQuizPlayer({ questions, topicTitle, onQuizCompleted }: QuizPlayerProps) {
  ...
  const handleSubmitQuiz = async () => {
    ...
    for (const q of questions) {
      ...
      if (q.type === "MCQ") {
        // Option selection check (e.g., 'A' or matching option string)
  ...
  return (
    <div className="glass-card p-6 sm:p-8 rounded-3xl border border-amber-500/30 space-y-6 bg-slate-900/90 shadow-2xl">
      ...
      {results && (
        <div className={`p-6 rounded-2xl border ${results.passed ? "bg-emerald-500/10 border-emerald-500/40 text-emerald-300" : "bg-red-500/10 border-red-500/40 text-red-300"} space-y-3`}>
          ...
          <p className="text-xs text-slate-300">
            {results.passed
              ...
              : "Minimum score of 70% required to issue course certificate. Review reading materials and re-take exam."}
          </p>
        </div>
      )}
