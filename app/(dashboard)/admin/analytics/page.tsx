      ...
      <div className="glass-card p-6 rounded-3xl border border-amber-500/30 bg-slate-900/90 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            ...
            <span>AI District Skill Heatmap & NVIDIA NIM Trend Insights</span>
          </h1>
        ...
        <div className="glass-card p-6 rounded-3xl border border-amber-500/40 space-y-4 bg-slate-900/90 animate-fade-in">
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            <span className="text-xs font-extrabold text-amber-300 flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-amber-400" /> AI DISTRICT WORKFORCE INSIGHTS ({insights.district})
            </span>
            <span className="text-sm font-black text-cyan-300">{insights.placementRatePercentage || 92}% Placement Rate</span>
          </div>

          <div className="space-y-3">
            <h4 className="text-xs font-bold text-white uppercase">Policy & Allocation Recommendations</h4>
            {insights.aiPolicyRecommendations?.map((rec: string, i: number) => (
              <div key={i} className="p-3.5 rounded-2xl bg-white/5 border border-white/10 text-xs text-slate-200">
                💡 {rec}
              </div>
            ))}
          </div>
